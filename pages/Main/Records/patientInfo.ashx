...
public class patientInfo : IHttpHandler
{
    ...
    private string getfixrecordinfo(HttpContext context)
    {
        ...
        string sqlCommand = "select part.Name as partname,patient.*, subcenterprincipal.Name as subname,subcenterprincipal.Hospital as hos,subcenterprincipal.ID as subid,user.Name as username from user,part,subcenterprincipal,treatment,patient,diagnosisrecord where patient.RegisterDoctor=user.ID and patient.ID=treatment.Patient_ID and treatment.DiagnosisRecord_ID=diagnosisrecord.ID and patient.SickPart=part.ID and diagnosisrecord.SubCenterPrincipal_ID=subcenterprincipal.ID and treatment.ID=@treatID";
        ...
        while (reader.Read())
        {
            ...
            backText.Append("{\"ID\":\"" + reader["ID"].ToString() + "\",\"IDcardNumber\":\"" + reader["IdentificationNumber"] + "\",\"Hospital\":\"" + reader["Hospital"] +
                 ...
                 "\",\"Nation\":\"" + reader["Nation"].ToString() + "\",\"Address\":\"" + reader["Address"].ToString() + "\",\"Contact1\":\"" + reader["Contact1"].ToString() + "\",\"doctor\":\"" + reader["RegisterDoctor"].ToString() + "\",\"doctorname\":\"" + reader["username"].ToString() +
                 "\",\"Contact2\":\"" + reader["Contact2"].ToString() + "\",\"Height\":\"" + reader["Height"].ToString() + "\",\"Weight\":\"" + reader["Weight"].ToString() + "\",\"SickPart\":\"" + reader["partname"].ToString() + "\",\"SickPartID\":\"" + reader["SickPart"].ToString() + "\",\"date\":\"" + reader["RegisterTime"].ToString() + "\"}");
