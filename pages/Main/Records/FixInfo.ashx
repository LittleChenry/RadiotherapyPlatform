...
public class FixInfo : IHttpHandler
{
    ...
    private string getfixrecordinfo(HttpContext context)
    {
            ...
            int i = 1;
            string sqlCommand1 = "select Treatmentname,Treatmentdescribe,fixed.*,material.Name as mname,headrest.Name as headname,fixed.ID as fixedid,user.Name as doctor,fixedequipment.Name as fename,fixedrequirements.* from headrest,fixedequipment,user,material,fixedrequirements,treatment,fixed where headrest.ID=fixed.HeadRest_ID and fixed.Application_User_ID=user.ID and fixedequipment.ID=fixed.FixedEquipment_ID and fixed.Model_ID=material.ID and fixed.FixedRequirements_ID=fixedrequirements.ID and treatment.Fixed_ID=fixed.ID and treatment.Patient_ID=@patient";
            ...
            while (reader.Read())
            {
                ...
                backText.Append("{\"modelID\":\"" + reader["Model_ID"].ToString() + "\",\"requireID\":\"" + reader["FixedRequirements_ID"].ToString() + "\",\"Treatmentname\":\"" + reader["Treatmentname"].ToString() +
                     ...
                     "\",\"AnnexDescription\":\"" + reader["AnnexDescription"].ToString() + "\",\"Remarks\":\"" + reader["Remarks"].ToString() + "\",\"Pictures\":\"" + reader["Pictures"].ToString() + "\",\"OperateTime\":\"" + date2 + "\",\"headrestname\":\"" + reader["headname"].ToString() +
