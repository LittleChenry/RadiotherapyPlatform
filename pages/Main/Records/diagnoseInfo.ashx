...
public class diagnoseInfo : IHttpHandler {
    ...
    private string getfixrecordinfo(HttpContext context)
    {
        ...
        int i = 1;
        string sqlCommand1 = "select diagnosisrecord.*,part.Name as partname,user.Name as username,Group_ID,groups.groupName as groupName,Treatmentname,TumorName,diagnosisresult.Description as des from groups,user,part,diagnosisrecord,treatment,diagnosisresult where diagnosisresult.ID=diagnosisrecord.DiagnosisResult_ID and groups.ID=treatment.Group_ID and diagnosisrecord.Diagnosis_User_ID=user.ID and treatment.DiagnosisRecord_ID=diagnosisrecord.ID and diagnosisrecord.Part_ID=part.ID and treatment.Patient_ID=@patient";
        sqlOperation2.AddParameterWithValue("@patient", patientid);
        ...
        while (reader.Read())
        {
            ...
            backText.Append("{\"Remarks\":\"" + reader["Remarks"].ToString() + "\",\"partname\":\"" + reader["partname"] + "\",\"partID\":\"" + reader["Part_ID"] + "\",\"Treatmentname\":\"" + reader["Treatmentname"] + "\",\"diagnosisresultName\":\"" + reader["TumorName"].ToString() + reader["des"].ToString() +
                 "\",\"diagnosisresultID\":\"" + reader["DiagnosisResult_ID"].ToString() + "\",\"username\":\"" + reader["username"].ToString() + "\",\"group\":\"" + reader["Group_ID"].ToString() + "\",\"groupName\":\"" + reader["groupName"].ToString() + "\",\"userID\":\"" + reader["Diagnosis_User_ID"].ToString() +
                 "\",\"Time\":\"" + date1 + "\"}");
            if (i < count)
            {
                backText.Append(",");
