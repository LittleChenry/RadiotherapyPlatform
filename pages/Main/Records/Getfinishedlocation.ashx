...
public class Getfinishedlocation : IHttpHandler {
    ...
    public string patientLocationInformation(HttpContext context)
    {
        ...
        string sqlcommand2 = "select distinct(count(*)) from treatment,location,scanmethod,locationrequirements,enhancemethod,user,appointment,equipment where treatment.Patient_ID=@patient and treatment.Location_ID=location.ID and location.Appointment_ID=appointment.ID  and  location.ScanMethod_ID=scanmethod.ID  and location.LocationRequirements_ID=locationrequirements.ID and location.Application_User_ID=user.ID and appointment.Equipment_ID=equipment.ID";
        ...
        while (reader.Read())
        {
            ...
            backText.Append("{\"scanmethod\":\"" + reader["scanmethod"].ToString() + "\",\"treatname\":\"" + reader["treatname"] + "\",\"scanmethodID\":\"" + reader["scanmethodID"] + "\",\"scanpartID\":\"" + reader["scanpartID"] + "\",\"locationrequireID\":\"" + reader["locationrequireID"] +
                 "\",\"UpperBound\":\"" + reader["UpperBound"].ToString() + "\",\"LowerBound\":\"" + reader["LowerBound"].ToString() + "\",\"equipname\":\"" + reader["equipname"].ToString() + "\",\"appointid\":\"" + reader["Appointment_ID"].ToString() + "\",\"userID\":\"" + reader["Application_User_ID"].ToString() +
