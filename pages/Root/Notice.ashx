...
public class Notice : IHttpHandler {
    ...
    private string getTitle(HttpContext context)
    {
        ...
        string sqlCommand = "SELECT news.ID as newsid,Title,Releasetime,Important,user.name as username FROM news,user WHERE user.ID=news.Release_User_ID and Permission like(@type) order by Important desc, Releasetime desc";
        ...
        while (reader.Read())
        {
            ...
            backText.Append("{\"Title\":\"" + reader["Title"].ToString() + "\",\"Time\":\"" + day
                + "\",\"ID\":\"" + reader["newsid"].ToString() + "\",\"Important\":\"" + reader["Important"].ToString() + "\",\"ReleaseUserName\":\"" + reader["username"].ToString() + "\"}");
            if (i < count)
            {
                //backText += ",";
                backText.Append(",");
