    ...
    if (designInfo.length > 1) {
        for (var i = 1; i < designInfo.length; i++) {
            ...
            var content = '<div class="tab-pane" id="tab' + i + '"><div class="single-row"><input type="hidden"  id="TechnologyConfirm1_' + i + '" name="TechnologyConfirm1_' + i + '" value="0"/>' +
                          ...
                          '<button id="save_' + i + '" type="button" class="btn btn-success" disabled="disabled" onclick="savereview(' + i + ')">保存</button>' +
                          '<button id="Forced_' + i + '" type="button" class="btn btn-success" onclick="force('+i+')" disabled="disabled">强制通过</button>' +
